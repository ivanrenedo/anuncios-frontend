"use client";

import Link from "next/link";
import { useRouter } from "next/navigation";
import { useState } from "react";
import {
  Search,
  Bell,
  Heart,
  Plus,
  LogIn,
  LayoutGrid,
  User,
} from "lucide-react";
import { useAuth } from "@/hooks/useAuth";
import { resolveImage } from "@/lib/config";
import ThemeToggle from "./ThemeToggle";

export default function Header() {
  const router = useRouter();
  const { isAuthenticated, user } = useAuth();
  const [q, setQ] = useState("");
/* console.log(user) */
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    router.push(`/explore?q=${encodeURIComponent(q.trim())}`);
  };

  return (
    <header className="sticky top-0 z-50 border-b border-outline-variant/40 bg-surface/85 backdrop-blur-xl">
      <div className="mx-auto flex h-14 max-w-7xl items-center gap-3 px-4 sm:h-16 sm:px-6">
        {/* Logo */}
        <Link href="/" className="flex shrink-0 items-center gap-2">
          <span className="grid h-8 w-8 place-items-center rounded-xl bg-primary text-sm font-extrabold text-on-primary">
            M
          </span>
          <span className="hidden text-lg font-extrabold tracking-tight text-on-surface sm:block">
            Market EG
          </span>
        </Link>

        {/* Search (desktop) */}
        <form
          onSubmit={submit}
          className="relative hidden flex-1 sm:block"
        >
          <Search
            size={18}
            className="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-muted"
          />
          <input
            value={q}
            onChange={(e) => setQ(e.target.value)}
            placeholder="Buscar en Market EG…"
            className="h-10 w-full rounded-full border border-outline-variant/50 bg-surface-lowest pl-10 pr-4 text-sm outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/20"
          />
        </form>

        {/* Desktop nav */}
        <nav className="hidden items-center gap-1 sm:flex">
          <Link
            href="/categories"
            className="flex items-center gap-1.5 rounded-full px-3 py-2 text-sm font-semibold text-on-surface-variant transition hover:bg-surface-container"
          >
            <LayoutGrid size={18} strokeWidth={1.7} />
            Categorías
          </Link>
          <Link
            href="/saved"
            className="flex items-center gap-1.5 rounded-full px-3 py-2 text-sm font-semibold text-on-surface-variant transition hover:bg-surface-container"
          >
            <Heart size={18} strokeWidth={1.7} />
            Favoritos
          </Link>
          <ThemeToggle />
          {isAuthenticated && (
            <Link
              href="/notifications"
              className="grid h-10 w-10 place-items-center rounded-full text-on-surface-variant transition hover:bg-surface-container"
              aria-label="Notificaciones"
            >
              <Bell size={20} strokeWidth={1.6} />
            </Link>
          )}
          <Link
            href="/post"
            className="ml-1 flex items-center gap-1.5 rounded-full bg-primary px-4 py-2 text-sm font-bold text-on-primary shadow-soft transition hover:opacity-90"
          >
            <Plus size={18} strokeWidth={2.4} />
            Vender
          </Link>
          {isAuthenticated ? (
            <Link
              href="/profile"
              className="ml-1 grid h-10 w-10 place-items-center overflow-hidden rounded-full border border-outline-variant/50"
              aria-label="Perfil"
            >
              {user?.avatarUrl ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img
                  src={resolveImage(user.avatarUrl)}
                  alt=""
                  className="h-full w-full object-cover"
                />
              ) : (
                <User size={20} className="text-on-surface-variant" />
              )}
            </Link>
          ) : (
            <Link
              href="/login"
              className="ml-1 flex items-center gap-1.5 rounded-full bg-surface-container px-4 py-2 text-sm font-bold text-primary transition hover:bg-surface-high"
            >
              <LogIn size={16} strokeWidth={2} />
              Entrar
            </Link>
          )}
        </nav>

        {/* Mobile right actions */}
        <div className="ml-auto flex items-center gap-1 sm:hidden">
          <ThemeToggle />
          <Link
            href="/explore"
            className="grid h-10 w-10 place-items-center rounded-full text-primary"
            aria-label="Buscar"
          >
            <Search size={22} strokeWidth={1.8} />
          </Link>
          {isAuthenticated ? (
            <Link
              href="/notifications"
              className="grid h-10 w-10 place-items-center rounded-full text-primary"
              aria-label="Notificaciones"
            >
              <Bell size={22} strokeWidth={1.6} />
            </Link>
          ) : (
            <Link
              href="/login"
              className="flex items-center gap-1.5 rounded-full bg-primary px-3.5 py-2 text-[13px] font-bold text-on-primary"
            >
              <LogIn size={15} strokeWidth={2} />
              Entrar
            </Link>
          )}
        </div>
      </div>
    </header>
  );
}
